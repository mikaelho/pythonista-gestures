...
'''
# Gestures for Pythonista iOS app
...
correspond to the standard Apple gestures, except for the custom force press
gesture.
...
information; see the API documentation for the `add_` methods.
...
* `began`, `changed`, `ended` - convenience boolean properties to check for
  these states
...
All of the `add_x` methods return a `recognizer` object that can be used to
remove or disable the gesture as needed, see the API. You can also remove
...
By default only one gesture recognizer will be successful, but if you want
to, for example, enable both zooming (pinch) and panning at the same time,
allow both recognizers:

    g = Gestures()
    
    g.recognize_simultaneously = lambda gr, other_gr: (
        gr == Gestures.PAN and other_gr == Gestures.PINCH
    )
    
The other methods you can override are `fail` and `fail_other`,
corresponding to the other [UIGestureRecognizerDelegate](
https://developer.apple.com/reference/uikit/uigesturerecognizerdelegate
?language=objc) methods.
    
All regular recognizers have convenience names that you can use like in the
example above: `gestures.TAP/PINCH/ROTATION/SWIPE/PAN/SCREEN_EDGE_PAN
/LONG_PRESS`.
...
"magic" `Gestures.close_app` method as the gesture handler. For example,
...
* It can be hard to add gestures to ui.ScrollView, ui.TextView and the like,
  ...
  place.
* To facilitate the gesture handler callbacks from Objective-C to Python, the
  Gestures instance used to create the gesture must be live. You do not need
  to manage that as objc_util.retain_global is used to keep a global
  reference around. If you for some reason must track the reference manually,
  you can turn this behavior off with a `retain_global_reference=False`
  parameter for the constructor.
* Single Gestures instance can be used to add any number of gestures to any
  number of views, but you can just as well create a new instance whenever
  and wherever you need to add a new handler.
...
class Gestures():
    ...
    def __init__(self,
        ...
        def gestureRecognizer_shouldBeRequiredToFailByGestureRecognizer_(
                _self, _sel, gr, other_gr):
        ...
        try:
            ...
            PythonistaGestureDelegate = create_objc_class(
                'PythonistaGestureDelegate',
                superclass=NSObject,
                methods=[
    ...
    def add_tap(self, view, action,
        ...
        * `number_of_touches_required` - Set if more than one finger is
        ...
        """
        recog = self._get_recog('UITapGestureRecognizer', view,
            self._general_action, action)
    ...
    def add_long_press(self, view, action,
        ...
        * `allowable_movement` - Set to change the default 10 point maximum
        ...
        """
        recog = self._get_recog('UILongPressGestureRecognizer', view,
            self._general_action, action)
    ...
    def add_pan(self, view, action,
        ...
        * `velocity` - Current velocity of the pan gesture as points per
        ...
        """
        recog = self._get_recog('UIPanGestureRecognizer', view,
            self._pan_action, action)
    ...
    def add_screen_edge_pan(self, view, action, edges):
        ...
        the `data` argument as pan gestures, see `add_pan`.
        """
        recog = self._get_recog('UIScreenEdgePanGestureRecognizer', view,
            self._pan_action, action)
    ...
    def add_pinch(self, view, action):
        ...
        * `velocity` - Current velocity of the pinch gesture as scale
          per second.
        """
        recog = self._get_recog('UIPinchGestureRecognizer', view,
            self._pinch_action, action)
    ...
    def add_rotation(self, view, action):
        ...
        * `velocity` - Current velocity of the rotation gesture as radians
          per second.
        """
        recog = self._get_recog('UIRotationGestureRecognizer', view,
            self._rotation_action, action)

        return recog

    @on_main_thread
    def add_swipe(self, view, action, direction=None,
            number_of_touches_required=None):
        ...
        * `number_of_touches_required` - Set if you need to change the minimum
        ...
        If swipes to multiple directions are to be recognized, the handler
        ...
        directions.
        """
        recog = self._get_recog('UISwipeGestureRecognizer', view,
            self._general_action, action)
    ...
    def add_force_press(self, view, action, threshold=0.4):
        """ Call `action` when a force press gesture is recognized for the
        `view`. This is a continuous gesture.

        Additional parameters:

        * `threshold` - How much pressure is required for the gesture to be
          detected, between 0 and 1. Default is 0.4.
        ...
        * `force` - Force of the press, a value between `threshold` and 1.
        """
        recog = self._get_recog('UILongPressGestureRecognizer', view,
            partial(self._force_press_action, threshold), action)
    ...
    def replace_close_gesture(cls, view, recog_name):
        ...
        recognizer = ObjCClass(recog_name).alloc().initWithTarget_action_(
    ...
    def _get_recog(self, recog_name, view, internal_action, final_handler):
        if not final_handler == Gestures.close_app:
            ...
            self.views[key] = view
            recognizer = ObjCClass(recog_name).alloc().initWithTarget_action_(
                button, sel('invokeAction:')).autorelease()
    ...
    class Data():
        ...
        @property
        def began(self):
            return self.state == Gestures.BEGAN

        @property
        def changed(self):
            return self.state == Gestures.CHANGED

        @property
        def ended(self):
            return self.state == Gestures.ENDED

    def _context(self, button):
        key = button.name
        (view, recog, action) = (
        self.views[key], self.recognizers[key], self.actions[key])
        data = Gestures.Data()
...
if __name__ == "__main__":
    ...
    g.fail_other = lambda gr, other_gr: (
        gr == Gestures.PAN and other_gr == Gestures.SWIPE
    )
